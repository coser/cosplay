<?php $this->load->view('/public/general-header'); ?>
<div class="general-pageheader">
	<div class="container">
		<h1 class="navigate">cos社团</h1>
		<a href="" class="actionbutton-gray">设置</a>
		<ul>
			<li><a href="">社团作品</a></li>
			<li><a href="">成员列表</a></li>
		</ul>
		<div class="clear"></div>
	</div>
</div>
<div class="container societies-info">
	<div class="left-block">
		<div class="general-title">
			<h3>社团公告</h3>
			<a href="" class="actionbutton-gray">编辑</a>	
		</div>
		<div class="societies-bulletin">
			星期6 活动地点 正大广场，时间下午2点
		</div>
		<div class="general-title">
			<h3>社团动态</h3>
		</div>
		<div class="general-feedlist">
			<ul>
				<li>
					<img class="avatar" src="/resource/images/test.jpg">
					<b>redback1020</b>
					<p>创建了社团<span>2014-4-2</span></p>
					<div class="clear"></div>
				</li>
				<li>
					<img class="avatar" src="/resource/images/test.jpg">
					<b>redback1020</b>
					<p>创建了社团创建了社团创建了社团创建了社团创建了<span>2014-4-2</span></p>
					<div class="clear"></div>
				</li>
				<li>
					<img class="avatar" src="/resource/images/test.jpg">
					<b>redback1020</b>
					<p>创建了社团<span>2014-4-2</span></p>
					<div class="clear"></div>
				</li>
				<li>
					<img class="avatar" src="/resource/images/test.jpg">
					<b>redback1020</b>
					<p>创建了社团<span>2014-4-2</span></p>
					<div class="clear"></div>
				</li>
				<li>
					<img class="avatar" src="/resource/images/test.jpg">
					<b>redback1020</b>
					<p>创建了社团<span>2014-4-2</span></p>
					<div class="clear"></div>
				</li>
			</ul>
		</div>
		<div class="general-title">
			<h3>最新作品</h3>
		</div>
		<div class="societies-show-newest">
			<ul>
				<li>
					<a href="/show/info"><img class="cover" src="/resource/images/test.jpg" /></a>
				</li>
				<li>
					<a href="/show/info"><img class="cover" src="/resource/images/test.jpg" /></a>
				</li>
				<li>
					<a href="/show/info"><img class="cover" src="/resource/images/test.jpg" /></a>
				</li>
				<li>
					<a href="/show/info"><img class="cover" src="/resource/images/test.jpg" /></a>
				</li>
				<li>
					<a href="/show/info"><img class="cover" src="/resource/images/test.jpg" /></a>
				</li>
				<li>
					<a href="/show/info"><img class="cover" src="/resource/images/test.jpg" /></a>
				</li>
				<div class="clear"></div>
			</ul>
		</div>
		<div class="general-title">
			<h3>最新加入</h3>
		</div>
		<div class="societies-userlist">
			<ul>
				<li>
					 <a class="img_text" href=""><img class="avatar" src="/resource/images/test.jpg">redback1020</a>
				</li>
				<li>
					<a class="img_text" href=""><img class="avatar" src="/resource/images/test.jpg">redback1020</a>
				</li>
				<li>
					<a class="img_text" href=""><img class="avatar" src="/resource/images/test.jpg">redback1020</a>
				</li>
				<li>
					<a class="img_text" href=""><img class="avatar" src="/resource/images/test.jpg">redback1020</a>
				</li>
				<li>
					<a class="img_text" href=""><img class="avatar" src="/resource/images/test.jpg">redback1020</a>
				</li>
				<li>
					<a class="img_text" href=""><img class="avatar" src="/resource/images/test.jpg">redback1020</a>
				</li>
				<li>
					<a class="img_text" href=""><img class="avatar" src="/resource/images/test.jpg">redback1020</a>
				</li>
				<li>
					<a class="img_text" href=""><img class="avatar" src="/resource/images/test.jpg">redback1020</a>
				</li>
				<li>
					<a class="img_text" href=""><img class="avatar" src="/resource/images/test.jpg">redback1020</a>
				</li>
				<li>
					<a class="img_text" href=""><img class="avatar" src="/resource/images/test.jpg">redback1020</a>
				</li>
				<li>
					<a class="img_text" href=""><img class="avatar" src="/resource/images/test.jpg">redback1020</a>
				</li>
				<li>
					<a class="img_text" href=""><img class="avatar" src="/resource/images/test.jpg">redback1020</a>
				</li>
				<div class="clear"></div>
			</ul>
		</div>
	</div>
	<div class="right-block">
		<div class="general-baseinfo-box">
			<div class="box-header">
				<img src="/resource/images/test.jpg">
				<h3>cos社团</h3>
			</div>
			<div class="box-body">
				<table>
					<tr>
						<td class="title">成员数</td>
						<td>444</td>
					</tr>
					<tr>
						<td class="title">作品数</td>
						<td>4564</td>
					</tr>
					<tr>
						<td class="title">团长</td>
						<td>
							<a class="img_text" href=""><img class="avatar" src="/resource/images/test.jpg">redback1020</a>
						</td>
					</tr>
				</table>
			</div>
		</div>
	</div>
	<div class="clear"></div>
</div>
<?php $this->load->view('/public/general-foot'); ?>	